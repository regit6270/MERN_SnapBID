...
export const getLiveAuctionsByCategory = async (req, res) => {
  const { category } = req.query;

  try {
    // Find auctions that match the category and have an end time greater than the current date-time
    ...
    res.status(500).json({ message: 'Failed to fetch live auctions', error });
  }
};
